-- Abstract Realms D1 Schema v2 (Safe Migration)
-- SAFE: uses IF NOT EXISTS — existing tables and data are PRESERVED
--
-- First-time deploy:
--   wrangler d1 execute merch-db --file=schema_v2.sql
--
-- Upgrading from schema v1 (if orders table is missing payment_screenshot_url):
--   Run this single statement first, then this file:
...
-- ── Seed products (safe to re-run — INSERT OR IGNORE skips duplicates) ────────
INSERT OR IGNORE INTO products (id, name, category, base_price, stock, is_active) VALUES
...
-- ── Seed keychain variants ────────────────────────────────────────────────────
INSERT OR IGNORE INTO product_variants (id, product_id, material, shape, price_modifier, stock) VALUES
